...
class ThirdorderWorkChain(BaseWorkChain):
    ...
    def define(cls, spec):
        ...
        spec.expose_inputs(
            ...
            namespace="thirdorder_sow",
            exclude=("structure",),
        )
        spec.expose_inputs(
            ThirdorderReapCalculation,
            namespace="thirdorder_reap",
            exclude=(
                "structure",
                "vasp_folder",
                "supercells_folder",
    ...
    def inspect_thirdorder_sow(self):
        ...
        if not calculation.is_finished_ok:
            self.report(
                "Thirdorder sow failed with exit status {}".format(
        ...
        try:
            for (
                supercell_name
            ) in self.ctx.supercells_folder.list_object_names():
    ...
    def run_vasp(self):
        from aiida_phonopy.common.builders import get_force_calcjob_inputs
        from aiida_phonopy.common.builders import get_calcjob_builder

        builder_inputs = get_force_calcjob_inputs(
            self.inputs.vasp_settings, self.ctx.current_structure
        )
        for key in self.ctx.supercells:
            builder = get_calcjob_builder(
                self.ctx.supercells[key],
                self.inputs.vasp_settings["forces"]["code_string"],
                builder_inputs,
                label=key,
            )
